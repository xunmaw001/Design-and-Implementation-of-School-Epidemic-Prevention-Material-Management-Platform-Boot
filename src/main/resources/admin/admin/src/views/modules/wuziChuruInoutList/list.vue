        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('wuziChuruInoutList','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('wuziChuruInoutList','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('wuziChuruInoutList','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('wuziChuruInoutList','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoyuanyiqingwuziguanli/upload/wuziChuruInoutListMuBan.xls"
                        >批量导入出入库详情数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('wuziChuruInoutList','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="wuziChuruInoutListUploadSuccess"
                                :on-error="wuziChuruInoutListUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('wuziChuruInoutList','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "wuziChuruInoutList.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('wuziChuruInoutList','查看')"
                          :data="dataList"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="wuziChuruInoutListNumber"
                                      header-align="center"
                                      label="操作数量">
                        <template slot-scope="scope">
                            {{scope.row.wuziChuruInoutListNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('wuziChuruInoutList','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('wuziChuruInoutList','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('wuziChuruInoutList','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                wuziChuruInoutId : null,
                wuziId : null,
                wuziChuruInoutListNumber : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '出入库类型': 'wuziChuruInoutTypes',
                //本表字段
                     '操作数量': "wuziChuruInoutListNumber",
                     '操作时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'wuzi_churu_inout_list',//当前表表名,
                        sumColum : 'wuzi_churu_inout_list_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'wuzi_churu_inout_list_name',//分组字符串字段
                        // types : 'wuzi_churu_inout_list_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "wuzi_churu_inout_list_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['wuziChuruInoutListDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            wuziChuruInoutListUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "wuziChuruInoutList/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            wuziChuruInoutListUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "wuziChuruInoutList/update",
                        method: "post",
                        data: {
                            id:id,
//                            wuziChuruInoutListTypes:wuziChuruInoutListTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
