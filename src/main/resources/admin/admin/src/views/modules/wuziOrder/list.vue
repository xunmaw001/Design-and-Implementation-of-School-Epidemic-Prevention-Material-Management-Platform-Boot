        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '订单类型' : ''">
                        <el-select v-model="searchForm.wuziOrderTypes" placeholder="请选择订单类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in wuziOrderTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('wuziOrder','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoyuanyiqingwuziguanli/upload/wuziOrderMuBan.xls"
                        >批量导入物资分配数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('wuziOrder','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="wuziOrderUploadSuccess"
                                :on-error="wuziOrderUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('wuziOrder','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入物资分配数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('wuziOrder','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "wuziOrder.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="wuziOrderTruePrice"
                                   header-align="center"
                                   label="实付价格">
                        <template slot-scope="scope">
                            {{scope.row.wuziOrderTruePrice}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="wuziOrderTypes"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('wuziOrder','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('wuziOrder','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('wuziOrder','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
            <el-button v-if="isAuth('wuziOrder','订单') && scope.row.wuziOrderTypes == 101 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-sold-out" size="mini" @click="refund(scope.row.id)">取消分配</el-button>
            <el-button v-if="isAuth('wuziOrder','订单') && scope.row.wuziOrderTypes == 101
&& sessionTable=='users'" type="primary" icon="el-icon-reading" size="mini"
                        @click="deliver(scope.row.id)"
            >分发</el-button>
            <el-button v-if="isAuth('wuziOrder','订单') && scope.row.wuziOrderTypes == 103 && sessionTable=='yonghu' && userId==scope.row.yonghuId" type="primary" icon="el-icon-reading" size="mini" @click="receiving(scope.row.id)">领取物资</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              wuziOrderTypesSelectSearch : [],
            form:{
                id : null,
                wuziId : null,
                yonghuId : null,
                buyNumber : null,
                wuziOrderTruePrice : null,
                wuziOrderTypes : null,
                insertTime : null,
                createTime : null,
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'wuzi_order',//当前表表名,
                        sumColum : 'wuzi_order_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'wuzi_order_name',//分组字符串字段
                        // types : 'wuzi_order_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.wuziOrderTypes!= '' && this.searchForm.wuziOrderTypes!= undefined) {
                    params['wuziOrderTypes'] = this.searchForm.wuziOrderTypes
                }
                
                params['wuziOrderDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=wuzi_order_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.wuziOrderTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            wuziOrderUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "wuziOrder/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            wuziOrderUploadError(data){
                this.$message.error('上传失败');
            },
	
			//取消分配
			refund(id){
				this.$confirm(`确定要取消分配吗?`, "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning"
				}).then(() => {
					this.$http({
						url: "wuziOrder/refund?id="+id,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							this.$message({
			...
			deliver(id){
				let _this = this;
				_this.$confirm(`确定要分发吗?`, "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning"
				}).then(() => {
					_this.$http({
						url: "wuziOrder/deliver?id="+id,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							_this.$message({
			...
			//领取物资
			receiving(id){
				this.$confirm(`确定要领取物资吗?`, "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning"
				}).then(() => {
					this.$http({
						url: "wuziOrder/receiving?id="+id,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							this.$message({
