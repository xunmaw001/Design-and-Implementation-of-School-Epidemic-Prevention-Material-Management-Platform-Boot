                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('wuzikucun','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/xiaoyuanyiqingwuziguanli/upload/wuzikucunMuBan.xls"
                        >批量导入物资库存数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('wuzikucun','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="wuzikucunUploadSuccess"
                                :on-error="wuzikucunUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('wuzikucun','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入物资库存数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('wuzikucun','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "wuzikucun.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="价格">
                        <template slot-scope="scope">
                            {{scope.row.wuziNewMoney}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="wuzikucunKucunNumber"
                                      header-align="center"
                                      label="物资库存">
                        <template slot-scope="scope">
                            {{scope.row.wuzikucunKucunNumber}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="wuzikucunAddress"
                                   header-align="center"
                                   label="存放位置">
                        <template slot-scope="scope">
                            {{scope.row.wuzikucunAddress}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="wuzikucunTypes"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('wuzikucun','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('wuzikucun','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('wuzikucun','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
                            <!--<el-button v-if="isAuth('wuzikucun','修改')" type="primary" icon="el-icon-add-location" size="mini" @click="plusWuzikucunKucunNumber(scope.row.id,scope.row.wuzikucunKucunNumber)">增加物资库存</el-button>-->
                            <!--<el-button v-if="isAuth('wuzikucun','修改')" type="primary" icon="el-icon-delete-location" size="mini" @click="reduceWuzikucunKucunNumber(scope.row.id,scope.row.wuzikucunKucunNumber)">减少物资库存</el-button>-->
...
<!-- 库存 -->
		<el-dialog
                :title="wuzikucunKucunNumberTitle"
                :visible.sync="wuzikucunKucunNumberVisible"
                width="30%">
            <span>操作数量</span><el-input-number v-model="kucunNumber" :min="1" :max="100" label="描述文字"></el-input-number>
            <span slot="footer" class="dialog-footer">
			<el-button @click="wuzikucunKucunNumberVisible = false">取 消</el-button>
			<el-button type="primary" @click="addWuzikucunKucunNumber()">确 定</el-button>
		  </span>
        </el-dialog>
...
<script>
    ...
    //当前表下拉框搜索条件
            form:{
                id : null,
                wuziId : null,
                wuzikucunKucunNumber : null,
                wuzikucunAddress : null,
                wuzikucunTypes : null,
                baozhiTime : null,
                wuzikucunDelete : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '价格': 'wuziNewMoney',
                //本表字段
                     '物资库存': "wuzikucunKucunNumber",
                     '存放位置': "wuzikucunAddress",
                     '物资状态': "wuzikucunValue",
                     '保质期': "baozhiTime",
                     '录入时间': "insertTime",
            },
        ...
        //库存
            ...
            //原有数量
            wuzikucunKucunNumber: 0,
            //标题
            wuzikucunKucunNumberTitle: null,
            //模态框状态
            wuzikucunKucunNumberVisible: false,
            };
        },
        created() {
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'wuzikucun',//当前表表名,
                        sumColum : 'wuzikucun_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'wuzikucun_name',//分组字符串字段
                        // types : 'wuzikucun_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['wuzikucunDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            wuzikucunUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "wuzikucun/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            wuzikucunUploadError(data){
                this.$message.error('上传失败');
            },
            //增加物资库存
			plusWuzikucunKucunNumber(thisId,wuzikucunKucunNumber){
			//清空数据
				//物资库存数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				//将模态框打开
				this.wuzikucunKucunNumberVisible = true;
				//标题
				this.wuzikucunKucunNumberTitle = '增加物资库存'
			},
			//减少物资库存
			reduceWuzikucunKucunNumber(thisId,wuzikucunKucunNumber){
			//清空数据
				//物资库存数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				this.wuzikucunKucunNumberVisible = true;
				//标题
				this.wuzikucunKucunNumberTitle = '减少物资库存'
			},
			//提交用户操作物资库存数量
			addWuzikucunKucunNumber(){
				var type = null
				var maxNumber = 0
				if(this.kucunNumberTypes == "+"){
					type = '增加'
					maxNumber = this.wuzikucunKucunNumber + this.kucunNumber
				}else{
					type = '减少'
					if(this.wuzikucunKucunNumber < this.kucunNumber || this.kucunNumber <= 0){
				...
				let date = {
					...
					"wuzikucunKucunNumber":maxNumber
				}
				...
				this.wuzikucunKucunNumberVisible = false;
			},	
            //无用
            wuyong(id) {
                let _this = this;
